# Spring Bootプロジェクト作成と起動確認

- レクチャーID: `sec-03-lec-01`
...
Spring Bootプロジェクトを作成し、最小のアプリケーションを起動する
...
レクチャーID: sec-03-lec-01
目的: Spring Bootプロジェクトを作成し、最小のアプリケーションを起動する
...
対象範囲（実際の分割はレビューで調整可）:
- backend/pom.xml
- backend/src/main/java/com/pocotech/aidd/Application.java
- backend/src/main/java/com/pocotech/aidd/api/HealthController.java
...
完了条件:
- 起動可能なSpring Bootプロジェクト

検証:
- ローカルでアプリが起動し、ヘルス確認が成功する
...
- `backend/pom.xml` — Spring Boot設定・コードの目安
- `backend/src/main/java/com/pocotech/aidd/Application.java` — Spring Boot設定・コードの目安
- `backend/src/main/java/com/pocotech/aidd/api/HealthController.java` — Spring Boot設定・コードの目安
- `backend/src/main/resources/application.yml` — Spring Boot設定・コードの目安
...
このレクチャー終了時のコミットメッセージは、レクチャー名と同じ **Spring Bootプロジェクト作成と起動確認** とする。
