# ReactプロジェクトとAPIクライアント

- レクチャーID: `sec-04-lec-01`
...
Reactプロジェクトを作成し、バックエンドAPIを呼び出す基盤を整える
...
レクチャーID: sec-04-lec-01
目的: Reactプロジェクトを作成し、バックエンドAPIを呼び出す基盤を整える
セクションの前提: 認証とAPI連携を後から追加できる、操作可能なReact画面を作る

依頼:
- 画面構成と環境変数の扱いを決める
- APIクライアントと型またはデータ形式を定義する
...
- `frontend/package.json` — React／Node成果物の目安
- `frontend/src/main.jsx` — React／Node成果物の目安
- `frontend/src/api.js` — React／Node成果物の目安
...
このレクチャー終了時のコミットメッセージは、レクチャー名と同じ **ReactプロジェクトとAPIクライアント** とする。
